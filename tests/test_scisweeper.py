...
class TestSciSweeper(unittest.TestCase):
    ...
    def test_sweeper(self):
        if os.name != "nt":
            self.ssw = SciSweeper(
                working_directory=os.path.join(file_location, "calc_test_sweeper")
            )
            self.ssw.job_class = BashSciSweeper
            self.ssw.job_name_function = job_name
            self.ssw.run_jobs_in_parallel(
                input_dict_lst=[{"value_1": 1, "value_2": 2, "value_3": 3}]
            )
            self.ssw.collect()
        ...
        if os.name != "nt":
            self.ssw = SciSweeper(
                working_directory=os.path.join(
                    file_location, "calc_test_sweeper_no_job_name"
                )
            )
            self.ssw.job_class = BashSciSweeper
            self.ssw.run_jobs_in_parallel(
    ...
    def test_collect_again(self):
        if os.name != "nt":
            self.ssw = SciSweeper(
                working_directory=os.path.join(file_location, "calc_test_collect")
            )
            self.ssw.job_class = BashSciSweeper
            self.ssw.run_job(
                job_working_directory=os.path.join(self.ssw.working_directory, "job_0"),
    ...
    def test_properties(self):
        self.ssw = SciSweeper(
            working_directory=os.path.join(file_location, "calc_test_property")
        )
        self.assertIsNone(self.ssw.pysqa)
        self.ssw.cores = 2
        self.assertEqual(self.ssw.cores, 2)
        self.assertEqual(len(self.ssw.broken_jobs), 0)
        self.ssw_job = SciSweeperJob(
            working_directory=os.path.join(file_location, "calc_test_job_property")
        )
        self.ssw_job.cores = 4
        self.assertEqual(self.ssw_job.cores, 4)
        self.assertIsNone(self.ssw_job.pysqa)
        self.assertEqual(
            self.ssw_job.working_directory,
            os.path.join(file_location, "calc_test_job_property"),
        )
        with self.assertRaises(NotImplementedError):
            self.ssw_job.executable
        with self.assertRaises(NotImplementedError):
            self.ssw_job.write_input(
                input_dict={"value_1": 1, "value_2": 2, "value_3": 3}
            )
        with self.assertRaises(NotImplementedError):
